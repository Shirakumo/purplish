...
(defun search-posts (query &key board thread (amount 20))
  (let* ((query (parse-search-query query))
         (query (cond
                  ((and thread board) (db:query (:and (:= 'board board) (:= 'parent thread)
                  ...
                  (T (db:query (:or (:matches 'title query)
                                    (:matches 'author query)
                                    (:matches 'text query)))))))
    (dm:get 'purplish-posts query :amount amount :sort '((time :DESC)))))
